                ...
                foreach (var model in models)
                {
                    foreach (var part in parts)
                    {
                        ...
                        using (var cmdInv = new SQLiteCommand(insertInventorySql, conn))
                        {
                            ...
                            // If INSERT OR IGNORE didn't insert, find existing StockID
                            if (stockId == 0)
                            {
                                stockId = (long)new SQLiteCommand(@"
                        ...
                        if (stockId > 0)
                        {
                            using var cmdInvLoc = new SQLiteCommand(
                                @"INSERT OR IGNORE INTO INVENTORY_LOCATIONS 
                                  (StockID, LocationID, Quantity, UnitType)
                                  VALUES (@StockID, @LocationID, @Quantity, @UnitType);", conn);
                            ...
                            cmdInvLoc.Parameters.AddWithValue("@UnitType", "pcs"); // default unit type
